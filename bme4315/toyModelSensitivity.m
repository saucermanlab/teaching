...
L = 1;   % [uM] total concentration of ligand
...
tspan = [0 100];
options = [];
[t,y] = ode23(@toyModelODEfunc,tspan,y0,options,params); % Run model with default params
...
for deltaNum=1:length(deltaParams)
    for paramNum = 1:length(params)    
        ...
        try
            [t,y] = ode23(@toyModelODEfunc,tspan,y0,options,paramsNew);
...
paramNames = {'k1f','k1r','k2f','k2r','kcat','Km','k4','Ltot','Rtot','Etot','Stot'};
...
tspan = [0 100];
options = [];
[t,y] = ode23(@toyModelODEfunc,tspan,y0,options,params); % run default simulation
y_fin_orig = y(end,:)';
for i=1:length(params)
    paramsNew = params; % Initialize new parameters to old ones
    y_initial = y0;
    paramsNew{i} = params{i}*1.1; % Increase parameter by 10%
    [~,y_new] = ode23(@toyModelODEfunc,tspan,y_initial,options,paramsNew);
...
yNames = {'R','LR','E','LRE','S','P'};
paramNames = {'k1f','k1r','k2f','k2r','kcat','Km','k4','L'};
