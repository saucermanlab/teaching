function motifsCoherentFeedforward() 
...
% dydt(B) = (OR(input,act(y(A),rpar(:,2)))*ymax(B) - y(B))/tau(B); % with OR feedforward
dydt(B) = (AND(input,act(y(A),rpar(:,2)))*ymax(B) - y(B))/tau(B); % with AND feedforward
% dydt(B) = (act(y(A),rpar(:,2))*ymax(B) - y(B))/tau(B); % without feedforward
