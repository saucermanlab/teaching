function motifsIncoherentFeedforward() 
...
dydt(B) = (AND(input,inhib(y(A),rpar(:,2)))*ymax(B) - y(B))/tau(B); % with feedforward
% dydt(B) = (input*ymax(B) - y(B))/tau(B); % without feedforward
