...
y0 = [Rtot; 0; Etot; 0; Stot; 0];   % or you could load saved data like: y0 = load('yfinal.dat');
tspan = [0 10];
options = [];%odeset('MaxStep',5e-3);
[t,y] = ode23(@toyModelODEfunc,tspan,y0,options,params);
...
for i=1:length(paramRange)
    ...
    tspan = [0 10];
    options = [];
    [t,y] = ode15s(@toyModelODEfunc,tspan,y0,options,params);
    Pfinal(i) = y(end,end);
end
subplot(1,3,3);
semilogx(paramRange,Pfinal);
xlabel('Ligand (\muM)'); ylabel('Steady state Product (\muM)');